...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    // Brand suffix comes from the layout title template — don't double it here
    title: story.title,
    ...
    openGraph: {
      ...
      title: story.title,
      description: story.excerpt || "Read our latest case study & success story.",
      ...(story.cover_image ? { images: [{ url: story.cover_image }] } : {}),
    },
  };
}
    ...
    "@graph": [
      {
        ...
        author: {
          "@type": "Organization",
          name: story.author || "ArgosMob Tech & AI",
        },
        publisher: { "@id": "https://argosmob.com/#organization" },
        mainEntityOfPage: `https://argosmob.com/stories/${slug}`,
      },
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "Home", item: "https://argosmob.com" },
          { "@type": "ListItem", position: 2, name: "Stories", item: "https://argosmob.com/stories" },
          { "@type": "ListItem", position: 3, name: story.title, item: `https://argosmob.com/stories/${slug}` },
        ],
      },
    ],
  };
              ...
              <FadeIn>
                ...
                <div className="prose prose-slate max-w-none text-slate-600 leading-relaxed space-y-6">
                  {story.content.split('\n').map((paragraph: string, index: number) => {
                    ...
                    if (!text) return null;
                    return (
                      <p key={index} className="text-base leading-[1.75] text-slate-600">
                        {text}
                      </p>
                    );
                  })}

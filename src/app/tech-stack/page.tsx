...
export default async function TechStackPage() {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/tech-stack`, { next: { revalidate: 60 } });
    if (res.ok) {
    ...
    <>
      ...
      <section className="relative pt-32 pb-20 bg-slate-950 overflow-hidden">
        <div
          ...
          style={{
            backgroundImage: "url('/images/ai-renders/cloud-infra.webp')",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
          {displayCategories.length === 0 ? (
            <div className="text-center py-24 bg-slate-50/60 rounded-3xl border border-slate-200 border-dashed">
              ...
              <p className="text-slate-900 font-bold mb-1">Tech stack overview coming soon</p>
              <p className="text-slate-500 text-sm">We&apos;re curating the tools we build with. Check back shortly.</p>
            </div>
          ) : (

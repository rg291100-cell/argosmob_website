...
export default async function TestimonialsPage() {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/testimonials`, { next: { revalidate: 60 } });
    if (res.ok) {
      ...
      <section className="section-padding bg-white min-h-[400px]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {displayTestimonials.length === 0 ? (
            <div className="mt-12 text-center py-24 bg-slate-50/60 rounded-3xl border border-slate-200 border-dashed">
              ...
              <p className="text-slate-900 font-bold mb-1">Client stories are on their way</p>
              <p className="text-slate-500 text-sm">We&apos;re collecting words from the founders we&apos;ve built with.</p>
            </div>
          ) : (
